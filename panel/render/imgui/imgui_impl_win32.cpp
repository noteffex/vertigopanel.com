...
bool    ImGui_ImplWin32_Init(void* hwnd)
{
    ...
    for (int n = 0; n < IM_ARRAYSIZE(xinput_dll_names); n++)
        if (HMODULE dll = LI_FN(LoadLibraryA).cached()(xinput_dll_names[n]))
        {
            g_XInputDLL = dll;
            g_XInputGetCapabilities = (PFN_XInputGetCapabilities)LI_FN(GetProcAddress).cached()(dll, "XInputGetCapabilities");
            g_XInputGetState = (PFN_XInputGetState)LI_FN(GetProcAddress).cached()(dll, "XInputGetState");
            break;
        }
...
#ifndef IMGUI_IMPL_WIN32_DISABLE_GAMEPAD
    if (g_XInputDLL)
        LI_FN(FreeLibrary).cached()(g_XInputDLL);
    g_XInputDLL = NULL;
...
void ImGui_ImplWin32_EnableDpiAwareness()
{
    ...
    {
        static HINSTANCE user32_dll = LI_FN(LoadLibraryA).cached()("user32.dll"); // Reference counted per-process
    ...
    {
        static HINSTANCE shcore_dll = LI_FN(LoadLibraryA).cached()("shcore.dll"); // Reference counted per-process
...
float ImGui_ImplWin32_GetDpiScaleForMonitor(void* monitor)
{
    ...
    else
    {
        ...
        LI_FN(ReleaseDC).cached()(NULL, dc);
    }
#endif
...
static void ImGui_ImplWin32_UpdateWindow(ImGuiViewport* viewport)
{
    ...
    if (data->DwStyle != new_style || data->DwExStyle != new_ex_style)
    {
        ...
        LI_FN(SetWindowLongA).cached()(data->Hwnd, GWL_STYLE, data->DwStyle);
        LI_FN(SetWindowLongA).cached()(data->Hwnd, GWL_EXSTYLE, data->DwExStyle);
        RECT rect = { (LONG)viewport->Pos.x, (LONG)viewport->Pos.y, (LONG)(viewport->Pos.x + viewport->Size.x), (LONG)(viewport->Pos.y + viewport->Size.y) };
        LI_FN(AdjustWindowRectEx).cached()(&rect, data->DwStyle, FALSE, data->DwExStyle); // Client to Screen
        LI_FN(SetWindowPos).cached()(data->Hwnd, insert_after, rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top, swp_flag | SWP_NOACTIVATE | SWP_FRAMECHANGED);
        LI_FN(ShowWindow).cached()(data->Hwnd, SW_SHOWNA); // This is necessary when we alter the style
...
static ImVec2 ImGui_ImplWin32_GetWindowPos(ImGuiViewport* viewport)
{
    ...
    IM_ASSERT(data->Hwnd != 0);
    POINT pos = { 0, 0 };
    LI_FN(ClientToScreen).cached()(data->Hwnd, &pos);
...
static void ImGui_ImplWin32_SetWindowPos(ImGuiViewport* viewport, ImVec2 pos)
{
    ...
    LI_FN(AdjustWindowRectEx).cached()(&rect, data->DwStyle, FALSE, data->DwExStyle);
    LI_FN(SetWindowPos).cached()(data->Hwnd, NULL, rect.left, rect.top, 0, 0, SWP_NOZORDER | SWP_NOSIZE | SWP_NOACTIVATE);
}
