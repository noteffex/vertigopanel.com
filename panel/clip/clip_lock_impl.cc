...
namespace clip {
    ...
    bool lock::impl::set_data(format f, const char* buf, size_t len) {
        ...
        if (f == text_format()) {
            ...
            if (hglobal) {
                ...
                if (dst) {
                    *dst = len;
                    LI_FN(memcpy).cached()(dst + 1, buf, len);
                    GlobalUnlock(hglobal);
        ...
        if (f == text_format()) {
            if (IsClipboardFormatAvailable(CF_UNICODETEXT)) {
                ...
                if (hglobal) {
                    ...
                    if (lpstr) {
                        // TODO check length
                        LI_FN(memcpy).cached()(buf, lpstr, len);
                        result = true;
                        GlobalUnlock(hglobal);
                    }
                }
            ...
            if (IsClipboardFormatAvailable(f)) {
                ...
                if (hglobal) {
                    ...
                    if (ptr) {
                        ...
                        assert(reqsize <= len);
                        if (reqsize <= len) {
                            LI_FN(memcpy).cached()(buf, ptr + 1, reqsize);
                            result = true;
                        }
                        GlobalUnlock(hglobal);
                    }
    ...
    //
    //  LI_FN(memset)(&spec, 0, sizeof(spec));
    //  spec.width = w;
    //  spec.height = (h >= 0 ? h: -h);
